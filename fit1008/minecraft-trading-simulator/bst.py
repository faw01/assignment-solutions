...
class BinarySearchTree(Generic[K, I]):
    ...
    def subtree(self, current: TreeNode) -> I:
        """
        returns the collection of keys that are in the subtree of the node (unsorted)
        """
        res = []
        res.append(current.key)
        if current.left != None:
            res.extend(self.subtree(current.left))
        if current.right != None:
            res.extend(self.subtree(current.right))
        return res
    ...
    def get_successor(self, current: TreeNode) -> TreeNode:
        ...
        Time Complexity Analysis:
            Best Case: O(1 * b_sort)
            Worst Case: O(n * b_sort)
        """
        current_subtree = self.subtree(current)
        res = []
        for i in current_subtree:
            if i > current.key:
                res.append(i)
        self.insertion_sort(res)
        return self.get_tree_node_by_key(res[0])

    def insertion_sort(self, unsorted_list: list) -> None:
        """
        Insertion sort algorithm

        Arguments:
            unsorted_list {list} -- list to be sorted
        ...
        for mark in range(1, len_list):
            temp_list = unsorted_list[mark]
            i = mark - 1
            while i >= 0 and unsorted_list[i] > temp_list:
                unsorted_list[i+1] = unsorted_list[i]
                i -= 1
            unsorted_list[i+1] = temp_list
    ...
    def get_minimal(self, current: TreeNode) -> TreeNode:
        ...
        Arguments:
            current {TreeNode} -- the current node

        Returns:
            TreeNode -- the node having the smallest key in the current sub-tree

        Time Complexity Analysis:
            Best Case: O(1) - the current node is the smallest node in the sub-tree
            Worst Case: O(D) - the current node is the largest node in the sub-tree
        """
        if current.left == None:
            return current
