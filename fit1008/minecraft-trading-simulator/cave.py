...
class Cave:
    ...
    Attributes:
        ...
        material {Material} -- The material the cave contains.
        quantity {float} -- The amount of material the cave contains.
    ...
    def __init__(self, name: str, material: Material, quantity: float=0) -> None: 
        ...
        if name not in CAVE_NAMES:
            raise ValueError(f"{name} is an invalid cave name.")
        if quantity < 0:
    ...
    def add_quantity(self, amount: float) -> None:
        """
        Adds a quantity of material to the cave.

        Arguments:
            amount {float} -- The amount of material to add to the cave.
    ...
    def remove_quantity(self, amount: float) -> None:
        """
        Removes a quantity of material from the cave.
        
        Arguments:
            amount {float} -- The amount of material to remove from the cave.

        Precondition:
            amount <= self.quantity
        """
        if amount > self.quantity:
            raise ValueError("Invalid amount (must be <= cave quantity)")
        self.quantity -= amount

    def get_quantity(self) -> float:
        """
        Returns the available amount of material in the cave.

        Returns:
            float -- The available amount of material in the cave.
        """
        return self.quantity

    def get_max_quantity(self) -> float:
        """
        Returns the maximum amount of material in the cave.

        Returns:
            float -- The maximum amount of material in the cave.
        """
        return self.max_quantity

    def get_material(self) -> Material:
        """
        Returns the material in the cave.

        Returns:
            Material -- The material in the cave.
        """
    ...
    def __str__(self) -> str:
        ...
        Returns:
            str -- A string representation of the cave.
        """
        return f"<Cave: {self.name}. {self.max_quantity} of [{str(self.material)}], {self.quantity}>"

    @classmethod
    def random_cave(self, material_list: list[Material]) -> Cave:
        """
        Returns a random cave with a random amount of material.

        Arguments:
            material_list {list[Material]} -- A list of materials to choose from.

        Returns:
            Cave -- A random cave with a random amount of material.
        
        Precondition:
            material_list is not empty
        """
        if len(material_list) == 0:
            raise ValueError("Empty material list")

        cave_name = RandomGen.random_choice(CAVE_NAMES)
        cave_material = RandomGen.random_choice(material_list)
        cave_quantity = RandomGen.randint(0, 100)
        return Cave(cave_name, cave_material, cave_quantity)